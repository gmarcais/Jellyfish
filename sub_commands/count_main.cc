...
public:
  mer_counter(int nb_threads, size_t size, uint16_t key_len, uint16_t val_len, uint16_t reprobe_limit,
              PathIterator file_begin, PathIterator file_end) :
    ary_(size, key_len, val_len, nb_threads, reprobe_limit),
...
int count_main(int argc, char *argv[])
{
  ...
  mer_counter<file_vector::iterator> counter(args.threads_arg, args.size_arg, args.mer_len_arg, args.counter_len_arg,
                                             args.reprobes_arg, args.file_arg.begin(), args.file_arg.end());
