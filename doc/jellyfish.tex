...
\section{Synopsis}
\Prog{jellyfish count} \oOptArg{-o}{prefix} \oOptArg{-m}{merlength} \oOptArg{-t}{threads} \oOptArg{-s}{hashsize} \oOpt{--both-strands} \Arg{fasta} \oArg{fasta \Dots} \\
\Prog{jellyfish merge} \Arg{hash1} \Arg{hash2} \Dots \\
\Prog{jellyfish stats} \oOpt{--fasta} \Arg{hash} \\
...
known, e.g. in sequencing reads, using \Opt{--both-strands} makes the most sense.
...
\begin{description}
\item[\Opt{-o},\OptArg{--output=}{prefix}] Output file prefix. Results will be store
...
\item[\Opt{-m},\OptArg{--mer-len=}{merlength}] Length of mer to
  count. I.e. value of $k$ in $k$-mer.
\item[\Opt{-t},\OptArg{--threads=}{NB}] Number of threads.
\item[\Opt{-s},\OptArg{--size=}{hashsize}] Size of hash table. This
  will be rounded up to the next power of two.
\item[\Opt{--both-strands}] Collapse counters for a $k$-mer and its
...
\item[\Opt{-p},\OptArg{--reprobes=}{NB}] Maximum reprobe
  value. This determine the usage of the hash table (i.e. \% of
  entries used in hash) before being deemed full and written to disk.
\item[\OptArg{--timing=}{file}] Write detailed timing information to
  \File{file}.
\item[\Opt{--no-write}] Do not write result file. Used for timing only.
\item[\OptArg{--out-counter-len=}{LEN}] Length of the counter field in
...
\begin{description}
\item[\Opt{-c},\Opt{--column}] Print $k$-mers counts in column format: sequence count.
\item[\Opt{-f},\Opt{--fasta}] Print $k$-mers counts in fasta format. The header is the count.
\item[\Opt{-r},\Opt{--recompute}] Recompute statistics from the hash
...
\begin{description}
\item[\Opt{-h},\OptArg{--high=}{HIGH}] High count bucket value.
\item[\Opt{-i},\OptArg{--increment=}{INC}] Increment for bucket value.
\item[\Opt{-l},\OptArg{--low=}{LOW}] Low count bucket value.
\end{description}
